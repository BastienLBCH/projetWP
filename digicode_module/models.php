...
$DEST_DIRECTORY_DIGICODE_FILES = "/srv/data/web/vhosts/www.livret-accueil-numerique.fr/htdocs/wp-content/medias/";

// Base URL pour récupérer les fichiers
$BASE_URL_DIGICODE_FILES = "www.livret-accueil-numerique.fr/htdocs/wp-content/plugins/medias/";
...
function move_files_digicode($files) {

    global $DEST_DIRECTORY_DIGICODE_FILES;
    global $BASE_URL_DIGICODE_FILES;
    ...
    foreach($files as $key => $file) {
        ...
        $dest = $DEST_DIRECTORY_DIGICODE_FILES . $new_name;
        $fileURL = $BASE_URL_DIGICODE_FILES . $new_name;
...
function delete_files_digicode($data, $files) {
    global $DEST_DIRECTORY_DIGICODE_FILES;
    global $BASE_URL_DIGICODE_FILES;
    ...
    foreach($files_in_request as $file) {
        $file_absolute_path = $DEST_DIRECTORY_DIGICODE_FILES . $files_to_delete[$file];
        unlink($file_absolute_path);
    }
}


function db_create_module_digicode($data, $files=false){
    global $db;
    ...
    // Insère les fichiers
    if($files != false) {
        $files_urls = move_files_digicode($files);
...
function db_get_module_digicode($module, $module_id) {
    global $db;
...
function db_update_module_digicode($data, $files=false) {
    global $db;
    global $reserved_keys;
    ...
    if($files != false) {
        delete_files_digicode($data, $files);

        $uploaded_files = move_files_digicode($files);
...
function get_fichiers_columns_digicode($module){
    global $db;
...
function delete_register_files_digicode($columns, $module, $module_id) {
    global $db;
    global $DEST_DIRECTORY_DIGICODE_FILES;
    ...
    foreach(array_keys($result) as $key) {
        if(in_array($key, $columns)) {
            $file_to_delete = $DEST_DIRECTORY_DIGICODE_FILES . $result[$key];
...
function db_delete_module_digicode($data) {
    global $db;

    $module = $data["module"];

    $columns = get_fichiers_columns_digicode($module);
    
    delete_register_files_digicode($columns, $data["module"], $data["id"]);
...
function db_list_module_digicode($module){
    global $db;
...
if(isset($_POST["module"]) && isset($_POST["action"])) {
    $actions_mapping = [
        "create" => "db_create_module_digicode",
        "update" => "db_update_module_digicode",
        "delete" => "db_delete_module_digicode"
    ];
