...
function move_files_infodepart($files) {
...
function delete_files_infodepart($data, $files) {
    global $DEST_DIRECTORY_WIFI_FILES;
    global $BASE_URL_WIFI_FILES;
...
function db_create_module_infodepart($data, $files=false){
    global $db;
    ...
    // Insère les fichiers
    if($files != false) {
        $files_urls = move_files_infodepart($files);
...
function db_get_module_infodepart($module, $module_id) {
    global $db;
...
function db_update_module_infodepart($data, $files=false) {
    global $db;
    global $reserved_keys;
    ...
    if($files != false) {
        delete_files_infodepart($data, $files);

        $uploaded_files = move_files_infodepart($files);
...
function get_fichiers_columns_infodepart($module){
    global $db;
...
function delete_register_files_infodepart($columns, $module, $module_id) {
    global $db;
    global $DEST_DIRECTORY_WIFI_FILES;
...
function db_delete_module_infodepart($data) {
    global $db;

    $module = $data["module"];

    $columns = get_fichiers_columns_infodepart($module);
    
    delete_register_files_infodepart($columns, $data["module"], $data["id"]);
...
function db_list_module_infodepart($module){
    global $db;
...
if(isset($_POST["module"]) && isset($_POST["action"])) {
    $actions_mapping = [
        "create" => "db_create_module_infodepart",
        "update" => "db_update_module_infodepart",
        "delete" => "db_delete_module_infodepart"
    ];
