...
function move_files_electro($files) {
...
function delete_files_electro($data, $files) {
    global $DEST_DIRECTORY_WIFI_FILES;
    global $BASE_URL_WIFI_FILES;
...
function db_create_module_electro($data, $files=false){
    global $db;
    ...
    // Insère les fichiers
    if($files != false) {
        $files_urls = move_files_electro($files);
...
function db_get_module_electro($module, $module_id) {
    global $db;
...
function db_update_module_electro($data, $files=false) {
    global $db;
    global $reserved_keys;
    ...
    if($files != false) {
        delete_files_electro($data, $files);

        $uploaded_files = move_files_electro($files);
...
function get_fichiers_columns_electro($module){
    global $db;
...
function delete_register_files_electro($columns, $module, $module_id) {
    global $db;
    global $DEST_DIRECTORY_WIFI_FILES;
...
function db_delete_module_electro($data) {
    global $db;

    $module = $data["module"];

    $columns = get_fichiers_columns_electro($module);
    
    delete_register_files_electro($columns, $data["module"], $data["id"]);
...
function db_list_module_electro($module){
    global $db;
...
if(isset($_POST["module"]) && isset($_POST["action"])) {
    $actions_mapping = [
        "create" => "db_create_module_electro",
        "update" => "db_update_module_electro",
        "delete" => "db_delete_module_electro"
    ];
